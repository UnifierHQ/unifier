...
discord_urls = ['discord.gg','discord.com','discord.gift','discord.gifts','discordapp.com','dis.gd','steampowered.com','discordapp.net','discord.new','discordstatus.com']
...
def compare_urls(urls,threshold,custom_blacklist=[],custom_whitelist=[]):
    ...
    for url in urls:
        ...
        if t in whitelist or t in real_urls or t in custom_whitelist:
            results.append(RapidPhishResult({'url':url,'verdict':'safe','whitelist_or_real':True,'scans':{'full':[],'nosubd':[],'nontld':[]}}))
        else:            
            t_hash = encrypt_string(t)
            if t_hash in discord_bl:
                results.append(RapidPhishResult({'url':url,'verdict':'unsafe','whitelist_or_real':False,'scans':{'full':[],'nosubd':[],'nontld':[]},'in_blacklist': True}))
                continue
            ...
            for domain in real_urls:
                ...
                if similarity >= threshold and not similarity==1:
                    verdict = 'unsafe'
                    result.append(RapidPhishScan({'domain':domain,'similarity':similarity,'type':'full','positive':True}))
                else:
                    result.append(RapidPhishScan({'domain':domain,'similarity':similarity,'type':'full','positive':False}))
            ...
            for domain in real_urls:
                ...
                if similarity >= threshold and not similarity==1:
                    verdict = 'unsafe'
                    result.append(RapidPhishScan({'domain':domain,'similarity':similarity,'type':'nosubd','positive':True}))
                else:
                    result.append(RapidPhishScan({'domain':domain,'similarity':similarity,'type':'nosubd','positive':False}))
            ...
            for domain in real_urls:
                ...
                if similarity >= threshold and not similarity==1:
                    verdict = 'unsafe'
                    result.append(RapidPhishScan({'domain':domain,'similarity':similarity,'type':'nontld','positive':True}))
                else:
                    result.append(RapidPhishScan({'domain':domain,'similarity':similarity,'type':'nontld','positive':False}))
            
            results.append(RapidPhishResult({'url':url,'verdict':verdict,'whitelist_or_real':False,'scans':result}))
